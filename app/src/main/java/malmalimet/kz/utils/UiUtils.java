...
public class UiUtils {
    ...
    /**
     * Open a given URL in a browser tab (Chrome custom tabs is used, if available).
     *
...
//    }
    ...
    /**
     * Return a drawable for the error indicator for a TextField widget.
     *
     * @param context - any context.
     * @return The error indicator drawable.
     */
    public static Drawable getTextFieldErrorDrawable(Context context) {
        if (sTextFieldErrorDrawable == null) {
            Drawable icon = VectorDrawableCompat.create(context.getResources(), R.drawable.ic_error, null);
            int size = (int) convertDpToPx(context, 20);
            if (icon != null) {
                sTextFieldErrorDrawable = DrawableCompat.wrap(icon);
                sTextFieldErrorDrawable.setBounds(0, 0, size, size);
                DrawableCompat.setTint(sTextFieldErrorDrawable, ContextCompat.getColor(context, R.color.errorDefault));
            }
        }

        return sTextFieldErrorDrawable;
    }
