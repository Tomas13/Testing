...
abstract public class BaseDataBoundAdapter<T extends ViewDataBinding>
    ...
    public DataBoundViewHolder<T> onCreateViewHolder(ViewGroup parent, int viewType) {
        DataBoundViewHolder<T> vh = createCustomViewHolder(parent, viewType);
        if (vh == null) {
            vh = DataBoundViewHolder.create(parent, viewType);
        }
        vh.binding.addOnRebindCallback(mOnRebindCallback);
        return vh;
    }

    protected DataBoundViewHolder<T> createCustomViewHolder(ViewGroup parent, int viewType) {
        return null;
    }
